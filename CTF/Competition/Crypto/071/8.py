...
def fermat_factorization(n):
    ...
    while b * b != b2:
        ...
        b2 = a * a - n
        if b2 < 0:
            print(f"b2 is negative at iteration {count}")
            return None, None
        b = gmpy2.isqrt(b2)
        count += 1
        if count % 1000000 == 0:
...
def parallel_fermat(n, num_processes=cpu_count()):
...
if __name__ == '__main__':
    ...
    # Note: You should replace `m1_bytes` with the actual bytes from Task 1
    m1_bytes = b'...'  # Placeholder: replace with actual decrypted m1 bytes
